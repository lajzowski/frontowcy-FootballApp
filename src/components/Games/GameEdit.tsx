...
export const GameEdit = (props: Props) => {
  const { mutate } = useCreateMutation<Game>(`games/${props.game.id}`, 'PATCH');
  const [errorMessage, setErrorMessage] = useState<string>('');
  const [newGame, setNewGame] = useState<Game>(props.game);
      ...
      },
      {
        onSuccess: async (newGame) => {
          ...
          queryClient.setQueryData<Game[]>(['games'], (oldData) => [
            ...(oldData ? oldData : []).map((game) =>
              game.id === props.game.id ? newGame : game
            ),
          ]);
          props.onSave();
        },
